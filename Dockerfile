...
RUN sed -i 's/^error_reporting.*/error_reporting = E_ALL/g' /etc/php/7.0/apache2/php.ini && \
...
RUN sed -i 's/^Deny from all/Allow from all/g' /var/www/html/mutillidae/.htaccess && \
    ...
    sed -i 's/\/var\/www\/html/\/var\/www\/html\/mutillidae/g' /etc/apache2/sites-enabled/000-default.conf
